...
- **`src/features/`** — `projects/` (rail), `terminals/` (Terminal, ProjectView, SplitLayout, NewTerminalMenu), `blocks/` (CommandPalette — runs presets + cross-project "jump to terminal"), `attention/` (AttentionPeek, the ⌘J cross-project triage overlay), `overview/` (OverviewPanel, the ⌘⇧J global overview), `drawer/` (Drawer, PresetsPanel), `board/` (QueueBoard), `settings/` (SettingsPanel), `tools/` (ToolRunModal — external-tool GUI), `plan/LiveView.tsx` (플랜 뷰의 "라이브" 탭: 전 프로젝트의 실시간 세션 노드 + 툴 노드).
- **External tools (command center)**: `src/lib/tools.ts` holds data-only tool manifests — modes/options as a form schema, argv builder, and a `detect` regex. SpriteForge is built in; any other tool plugs in by shipping a `fleet-tool.json` at its root (**the integration policy — see `docs/EXTERNAL_TOOLS.md`**), connected+managed in the **Settings panel's "외부 툴" section** (list/connect/set-root/remove; `SettingsPanel.tsx`) and persisted raw in `FleetConfig.customTools` (parsed by `parseToolManifest`, merged over built-ins by `mergeManifests`). The LiveView only *uses* tools (per-project run buttons + `ToolRunModal`, live-tool nodes, job progress) — it no longer owns tool management. Fleet-launched runs become live `ToolJob`s (spawned by `tools.rs`, streamed as `tool-job-output`/`tool-job-exit` events); a claude session invoking a matching Bash command (seen via the PreToolUse hook) becomes a `liveTool` entry, drawn as a tool node hanging off that session's node in LiveView. Tool root dirs + last-used run configs persist in `FleetConfig.toolRoots`/`toolPresets`.
